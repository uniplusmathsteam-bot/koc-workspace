const CHAPTER_TITLE = "Laboratory Safety";
const CHAPTER_SUBTITLE = "HKDSE Chemistry · Topic 0";
...
function setCardImage(el, card, show) {
  if (show && card.image) {
    el.src = card.image.replace(/^\.\//, "");
    el.alt = card.imageAlt || "";
...
function renderCard() {
  ...
  if (!card) {
    ...
    els.cardFront.classList.remove("card-has-front-image");
    setCardImage(els.cardFrontImage, null, false);
    setCardImage(els.cardImage, null, false);
    return;
  }
  ...
  els.cardFront.classList.toggle("card-text-fit", !!card.frontCompact);
  if (card.frontCompact) {
    els.cardFront.classList.remove("card-text-long", "card-text-compact");
  }
  els.cardBack.classList.remove("card-text-long", "card-text-compact", "card-text-fit");
  if (card.backCompact) {
    els.cardBack.classList.add("card-text-fit");
  } else {
    applyCardTextSize(els.cardBack, card.back, true, "back");
  }

  const onFront = card.imageFace === "front";
  els.cardFront.classList.toggle("card-has-front-image", onFront && !!card.image);
  setCardImage(els.cardFrontImage, card, !!card.image && onFront);
  setCardImage(els.cardImage, card, !!card.image && !onFront);
  els.cardImage.classList.toggle("card-image-compact", !onFront && !!card.image && !!card.imageCompact);
...
function applyCardTextSize(el, text, isHtml = false, side = "front") {
  el.classList.remove("card-text-long", "card-text-compact", "card-text-fit");
